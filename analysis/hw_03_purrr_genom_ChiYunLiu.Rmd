...
```{r}
fpout_df <- readRDS("../data/fpout.RDS")
```
...
```{r}
fpout_df$scores %>% 
  mutate(posterior_mean = (P0 * 0) + (P1 * 1) + (P2 * 2) + (P3 * 3) + (P4 * 4) + (P5 * 5) + (P6 * 6)) -> fpout_df$scores
str(fpout_df$scores)
```
...
```{r}
# check type
# Another way---names(keep(fpout_df$scores, ~ typeof(.) != "double"))
# "marker", "MarkerName", "SampleName" are not type of double.
names(discard(fpout_df$scores, ~ typeof(.) == "double"))


fpout_df$scores %>% 
  keep(is.double) %>% 
  map_dbl(mean, na.rm = TRUE) -> fpout_df$col_means
fpout_df$col_means
```
...
```{r}
# input:  x <- c(X_1, X_2, X_3)
#         n <- integer
# output: X_n                    >>>>> function input/output

# condition: length(x) = 3
#            n is integer        >>>>> if_else condition

# formula: X_n = X_n-1 + (abs(X_n-3 - X_n-2) / 4)   >>>>> for_loop

### note: condition then formula
...
calcn <- function(x, n){
  ...
  calvec <- vector(mode = "integer", length = n) # specify the type
  
  for( i in seq_along(calvec)){  # i reflect n 
    if (i <= 3) {
      calvec[i] <- x[i]
    } else {
      calvec[i] <- calvec[i - 1] + (abs(calvec[i - 3] - calvec[i - 2]))/4
    } 
  }
  return(calvec[n])
}
...
4. The $p$-value is an important quantity in statistics. Use a for-loop to extract the $p$-values from each test in part 1 and combine these $p$-values into a single vector called `pvec_f`. Show the first 6 values.

```{r}
pvec_f <- vector("double", length = length(tlist))

for (i in 1:1000){
  pvec_f[i] <- tlist[i] %>%  map_dbl("p.value")
}
pvec_f[c(1:6)]
```
...
pvec_m <- map_dbl(.x = tlist, .f = "p.value")
pvec_m[c(1:6)]
```
...
```{r}
df_pvec <- as.data.frame(pvec_m)

ggplot(df_pvec, aes(sample = pvec_m)) +
  stat_qq() +
  stat_qq_line(color = "red", linetype = "dashed") +
  labs(x = "theoretical quantiles", y = "sample p-value quantiles" , title = "P-value Q-Q Plot")
```
* As the Q-Q plot above, we can see there have many observations in both tails. The left tail spreads above the straight line(very high values). The right tail spreads below the straight line(very low values.) Therefore, the $p$-values do exhibit a uniform distribution.
